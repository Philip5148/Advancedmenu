...
if self.config["embed_text"] and self.config["embed_text"].lower() != "none":
    # send embed if embed_text is set and not "None"
    ...
    # no embed_text -> just send the dropdown under previous message
    msgs, _ = await thread.reply(dummyMessage, self.config["anonymous_menu"])
    main_recipient_msg = None
